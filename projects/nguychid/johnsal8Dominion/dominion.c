...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  {
  case adventurer:
    runAdventurer(currentPlayer, handPos, state, drawntreasure, temphand, cardDrawn, z);
    return 0;
  ...
  case smithy:
    ...
    runSmithy(currentPlayer, handPos, state);

    return 0;

  case village:
    runVillage(currentPlayer, handPos, state);
    return 0;
  ...
  case great_hall:
    runGreat_hall(currentPlayer, handPos, state);
    return 0;
  ...
  case embargo:
  {
    int result = runEmbargo(currentPlayer, handPos, state, choice1);
    return result;
  }
  case outpost:
    //set outpost flag
  ...
  while (drawntreasure < 2)
    {
      ...
      if (cardDrawn == copper || cardDrawn == silver /*| cardDrawn == gold*/) // bug (remove comment to fix)
        drawntreasure++;
      else
      {
...
void runVillage(int currentPlayer, int handPos, struct gameState* state)
{
    ...
    //+2 Actions
    state->numActions = state->numActions + 3; // bug (should be `state->numActions + 2`)
...
void runGreat_hall(int currentPlayer, int handPos, struct gameState* state)
{
    ...
    //discard card from hand
    // discardCard(handPos, currentPlayer, state, 0); // bug (uncomment to fix)
}
